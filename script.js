...
function fill_in_answers(answers) {
    ...
    var location = document.querySelector('#location');  // This says make a new variable and find the HTML tag that has the ID of "home" 
    ...
    children.innerText = answers['children'];
    car.innerText = answers['car'];
    job.innerText = answers['job'];
    salary.innerText = answers['salary'];
    location.innerHTML = answers.location;  // Change the content of the element in the HTML doc with the id "home" to the "mash" value in answers 
    children.innerHTML = answers.children;  // Change the content of the element in the HTML doc with the id "career" to the "career" value in answers 
    car.innerHTML = answers.car;
    job.innerHTML = answers.job;
    salary.innerHTML = answers.salary;
}
...
function handle_submission(evt) {
    ...
    var answers = {
        ...
        'children': get_answer('children'),
        'car': get_answer('car'),
        'job': get_answer('job'),
        'salary': get_answer('salary')
    }
    // Fill in the answers
